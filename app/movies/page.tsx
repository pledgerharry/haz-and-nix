    ...
    <div style={{minHeight:'100vh',backgroundColor:'#F7F5F1',fontFamily:'system-ui,sans-serif',paddingBottom:'80px'}}>
      ...
      <div style={{padding:'0 16px'}}>
        ...
        <div style={{display:'flex',backgroundColor:'#E4E1DB',borderRadius:'12px',padding:'3px',marginBottom:'14px'}}>
          {([['towatch','To watch',unwatchedCount],['watched','Watched',watchedCount]] as const).map(([val, label, count]) => (
            <button
              key={val}
              onClick={() => setTab(val)}
              style={{flex:1,padding:'8px',borderRadius:'10px',border:'none',cursor:'pointer',fontSize:'12px',fontWeight:'600',backgroundColor:tab===val?'#263322':'transparent',color:tab===val?'#F68233':'#ADADB3'}}
            >
              {label} ({count})
            </button>
          ))}
